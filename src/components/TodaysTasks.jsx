...
let TodaysTasks = (props) => {
  ...
  var list = () => {
    ...
    return props.data.map((priority) => (
      priority.projects && priority.projects.map((proj) => (
        proj.tasks && proj.tasks.filter((task) => (task.today)).map((task, index) => (
          <li className='list-item' key={index + '_' + task.id} >
            <div className='title'>{priority.title + ' > ' + proj.title + ' > ' + task.title}</div>
